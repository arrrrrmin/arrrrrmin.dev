...
function Chart(){
    ...
    function render (width, data) {
        ...
        g.transition(t * 0.25)
            .attr("transform", `translate(${x(data.location)}, ${size})`)
            .transition(t * 0.5)
            .attr("transform", `translate(${x(data.location)}, ${size - x.bandwidth()})`)
            .selectAll("rect.eye").transition(t * 0.25).attr("x", (_, i) => eyeX[i]);

        svg
            .selectAll("rect.bar")
            .data(data.bins.map((b, i) => ({length: b.length, i: i})).filter((d, i) => i != data.location))
            .join(
...
<div class="card" style="aspect-ratio: 1; max-width: 640px;">
${resize((width) => chart.render(width, data))}
